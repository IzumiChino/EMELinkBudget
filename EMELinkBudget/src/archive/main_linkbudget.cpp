...
void printGeometryResults(const GeometryResults& geo) {
    ...
    std::cout << "  Moon RA: " << geo.moonRA_deg << " deg" << '\n';
    std::cout << "  Moon DEC: " << geo.moonDEC_deg << " deg" << '\n';
    std::cout << "  Moon Distance: " << geo.moonDistance_km << " km" << '\n';
    std::cout << "\n  TX Station View:" << '\n';
    std::cout << "    Azimuth: " << geo.moonAzimuth_TX_deg << " deg" << '\n';
    std::cout << "    Elevation: " << geo.moonElevation_TX_deg << " deg" << '\n';
    std::cout << "  RX Station View:" << '\n';
    std::cout << "    Azimuth: " << geo.moonAzimuth_RX_deg << " deg" << '\n';
    std::cout << "    Elevation: " << geo.moonElevation_RX_deg << " deg" << '\n';
    std::cout << "\n  Path Length: " << geo.totalPathLength_km << " km" << '\n';
    std::cout << "  Doppler Shift: " << geo.dopplerShift_Hz << " Hz" << '\n';
}

void printPathLossResults(const PathLossResults& loss) {
    printSubHeader("Path Loss Analysis");
    std::cout << std::fixed << std::setprecision(2);
    std::cout << "  Wavelength: " << loss.wavelength_m << " m" << '\n';
    std::cout << "  Free Space Loss: " << loss.freeSpaceLoss_dB << " dB" << '\n';
    std::cout << "  Lunar Scattering Loss: " << loss.lunarScatteringLoss_dB << " dB" << '\n';
    std::cout << "  Atmospheric Loss (TX): " << loss.atmosphericLoss_TX_dB << " dB" << '\n';
    std::cout << "  Atmospheric Loss (RX): " << loss.atmosphericLoss_RX_dB << " dB" << '\n';
    std::cout << "  Total Path Loss: " << loss.totalPathLoss_dB << " dB" << '\n';
}

void printPolarizationResults(const PolarizationResults& pol) {
    printSubHeader("Polarization Analysis");
    std::cout << std::fixed << std::setprecision(3);
    std::cout << "  Spatial Rotation: " << pol.spatialRotation_deg << " deg" << '\n';
    std::cout << "  Faraday Rotation (TX): " << pol.faradayRotation_TX_deg << " deg" << '\n';
    std::cout << "  Faraday Rotation (RX): " << pol.faradayRotation_RX_deg << " deg" << '\n';
    std::cout << "  Total Rotation: " << pol.totalRotation_deg << " deg" << '\n';
    std::cout << "  PLF: " << std::setprecision(6) << pol.PLF << '\n';
    std::cout << "  Polarization Loss: " << std::setprecision(2)
              << pol.polarizationLoss_dB << " dB" << '\n';
    std::cout << "  Efficiency: " << pol.polarizationEfficiency_percent << " %" << '\n';
}

void printNoiseResults(const NoiseResults& noise) {
    printSubHeader("Noise Analysis");
    std::cout << std::fixed << std::setprecision(1);
    std::cout << "  Sky Noise Temperature: " << noise.skyNoiseTemp_K << " K" << '\n';
    std::cout << "  Ground Spillover: " << noise.groundSpilloverTemp_K << " K" << '\n';
    std::cout << "  Antenna Noise: " << noise.antennaNoiseTemp_K << " K" << '\n';
    std::cout << "  Antenna Effective: " << noise.antennaEffectiveTemp_K << " K" << '\n';
    std::cout << "  Receiver Noise: " << noise.receiverNoiseTemp_K << " K" << '\n';
    std::cout << "  System Noise: " << noise.systemNoiseTemp_K << " K" << '\n';
...
void printSNRResults(const SNRResults& snr) {
    ...
    std::cout << std::fixed << std::setprecision(2);
    std::cout << "  Received Signal Power: " << snr.receivedSignalPower_dBm << " dBm" << '\n';
    std::cout << "  SNR: " << snr.SNR_dB << " dB" << '\n';
    std::cout << "  Fading Margin: " << snr.fadingMargin_dB << " dB" << '\n';
    std::cout << "  Effective SNR: " << snr.effectiveSNR_dB << " dB" << '\n';
    std::cout << "  Required SNR: " << snr.requiredSNR_dB << " dB" << '\n';
    std::cout << "  Link Margin: " << snr.linkMargin_dB << " dB" << '\n';
    std::cout << "  Link Status: " << (snr.linkViable ? "VIABLE" : "NOT VIABLE") << '\n';
}
...
int main() {
    ...
    std::cout << "  Frequency: " << params.frequency_MHz << " MHz ("
              << ParameterUtils::getFrequencyBand(params.frequency_MHz) << " band)" << '\n';
    std::cout << "  TX Power: " << params.txPower_dBm << " dBm" << '\n';
    std::cout << "  TX Gain: " << params.txGain_dBi << " dBi" << '\n';
    std::cout << "  RX Gain: " << params.rxGain_dBi << " dBi" << '\n';
    std::cout << "  RX NF: " << params.rxNoiseFigure_dB << " dB" << '\n';
    std::cout << "  Bandwidth: " << params.bandwidth_Hz << " Hz" << '\n';
    std::cout << '\n';
    ...
    std::cout << std::fixed << std::setprecision(2);
    std::cout << "  Total Path Loss: " << results.totalLoss_dB << " dB" << '\n';
    ...
    std::cout << "  Link Status: "
              << (results.snr.linkViable ? "✓ VIABLE" : "✗ NOT VIABLE") << '\n';
